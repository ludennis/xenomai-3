/*
 * aiex3.cpp
 *   Continuous hardware-timed analog input with optimized DMA
 *
 * aiex3 reads and scales analog data using hardware timing and transfers data
 * to the host using an optimized DMA buffer. After configuring the AI
 * subsystem's timing and channel parameters, aiex3 configures and starts the
 * DMA channel before sending a software start trigger. For 10 seconds, data is
 * read, scaled, and printed in chunks until aiex3 stops the timing engine,
 * shuts down DMA, and flushes the buffer. Finally, aiex3 restores the
 ...
 *   Clock rate      : 1 kHz sample clock; 200 kHz convert clock (MIO only)
 ...
 *   PFI0..15        : TTL sample clock (if using an external clock)
 *
...
void test(iBus* bus)
{
   ...
   // Timing parameters
   nAI::tAI_StartConvertSelMux_t sampClkSrc = nAI::kStartCnv_InternalTiming;      // Source: on-board clock
   const u32 onBoardSamplePeriod = 100000;                                        // Rate: 100 MHz / 100e3 = 1 kHz
   ...
   // Timing parameters
   tBoolean useSICounter; // Should the AI timing engine use the sample interval counter?
   u32 samplePeriod = 0;
   u32 sampleDelay = 2; // Wait N TB3 ticks after the start trigger before clocking (N must be >= 2)
   ...
   if (sampClkSrc == nAI::kStartCnv_InternalTiming)
   {
      // Use SI for internal sample clocking
      useSICounter = kTrue;
      // Set the sample clock period as N TB3 ticks (N must be >= numberOfChannels*convertPeriod)
      samplePeriod = onBoardSamplePeriod;
      // Wait N TB3 ticks after the start trigger before clocking (N must be >= 2)
      sampleDelay = 2;
   }
   else
   {
   ...
   // NOTE: If using an external clock, program the corresponding PFI
   //       line for input (see tTriggers::PFI_Direction_Register).
   ...
   timingConfig.setUseSICounter(useSICounter, status);
   if (sampClkSrc == nAI::kStartCnv_InternalTiming)
   {
   ...
   printf("Starting continuous %.2f-second hardware-timed analog measurement.\n", runTime);
