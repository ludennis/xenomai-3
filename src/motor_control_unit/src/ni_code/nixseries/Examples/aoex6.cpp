/*
 * aoex6.cpp
 *   Continuous hardware-timed analog output with optimized DMA
 *
 * aoex6 scales and generates analog data using hardware timing and transfers
 * data to the device using an optimized DMA buffer. After configuring the AO
 * subsystem's timing and channels, aoex6 creates a sine wave and a square wave
 * to generate on both channels (with a 90-degree phase difference between
 * ao0 and ao1), and configures and primes the DMA channel before sending a
 * software start trigger. For 10 seconds, data is written to the buffer and
 * generated by the device, alternating between sine and square wave
 * waveforms until aoex6 stops the timing engine and shuts down DMA. Finally,
 * aoex6 restores the hardware's previous state.
 *
 ...
 * ! DMA buffer      : regenerate old data or wait for new data (*)
 * Behavior
 *   Runtime         : 10 seconds
 *
...
void test(iBus* bus)
{
   ...
   // Timing parameters
   const u32 samplePeriod = 20000; // Rate: 100 MHz / 20e3 = 5 kHz
   ...
   u32 writeCount = 0;     // How many times has data been written to the buffer?
   ...
   std::vector<f32> jointVoltData1;
   std::vector<f32> jointVoltData2;
   std::vector<i16> scaledData1(totalNumberOfSamples, 0);
   std::vector<i16> scaledData2(totalNumberOfSamples, 0);
   ...
   // Create the sine wave, with a 90-degree phase difference for ao1
   nNISTC3::nAODataHelper::generateSignal(nNISTC3::nAODataHelper::kSineWave, -1, 1, voltData1, sampsPerChan);
   std::vector<f32> phasedVoltData1(voltData1);
   std::rotate(phasedVoltData1.begin(), phasedVoltData1.begin()+sampsPerChan/4, phasedVoltData1.end());

   // Create the square wave, with a 90-degree phase difference for ao1
   nNISTC3::nAODataHelper::generateSignal(nNISTC3::nAODataHelper::kSquareWave, -2.5, 2.5, voltData2, sampsPerChan);
   std::vector<f32> phasedVoltData2(voltData2);
   std::rotate(phasedVoltData2.begin(), phasedVoltData2.begin()+sampsPerChan/4, phasedVoltData2.end());

   // Interleave both channels' data into a single buffer (ao0#0, ao1#0; ao0#1, ao1#1, ...)
   nNISTC3::nAODataHelper::interleaveData(sampsPerChan, numberOfChannels, jointVoltData1, voltData1, phasedVoltData1); // The sine wave
   nNISTC3::nAODataHelper::interleaveData(sampsPerChan, numberOfChannels, jointVoltData2, voltData2, phasedVoltData2); // The square wave

   // Scale both waveforms
   nNISTC3::nAODataHelper::scaleData(jointVoltData1, totalNumberOfSamples,
                                     scaledData1, totalNumberOfSamples,
                                     chanConfig, eepromHelper);
   nNISTC3::nAODataHelper::scaleData(jointVoltData2, totalNumberOfSamples,
                                     scaledData2, totalNumberOfSamples,
                                     chanConfig, eepromHelper);

   // Print both waveforms
   printf("Waveform set 1:\n");
   nNISTC3::nAODataHelper::printHeader(1, numberOfChannels, printVolts ? "Voltage" : "DAC code");
   if (printVolts)
   {
      nNISTC3::nAODataHelper::printData(jointVoltData1, totalNumberOfSamples, numberOfChannels);
   }
   else
   {
      nNISTC3::nAODataHelper::printData(scaledData1, totalNumberOfSamples, numberOfChannels);
   }
   printf("\n");

   printf("\n");
   printf("Waveform set 2:\n");
   nNISTC3::nAODataHelper::printHeader(1, numberOfChannels, printVolts ? "Voltage" : "DAC code");
   if (printVolts)
   {
      nNISTC3::nAODataHelper::printData(jointVoltData2, totalNumberOfSamples, numberOfChannels);
   }
   else
   {
      nNISTC3::nAODataHelper::printData(scaledData2, totalNumberOfSamples, numberOfChannels);
   }
   printf("\n");
   ...
   {
      dma->write(writeSizeInBytes, reinterpret_cast<u8 *>(&scaledData1[0]), &availableSpace, allowRegeneration, &dataRegenerated, status);
      if (status.isFatal())
      {
   ...
   printf("Generating continuous hardware-timed analog updates for %.2f seconds.\n", runTime);
   ...
   while (elapsedTime < runTime)
   {
      ...
      {
         // Alternate generating the waveforms in scaledData1 and scaledData2 every 16 cycles
         if (writeCount%32 < 16)
         {
            dma->write(writeSizeInBytes, reinterpret_cast<u8 *>(&scaledData1[0]), &availableSpace, allowRegeneration, &dataRegenerated, status);
         }
         ...
         {
            dma->write(writeSizeInBytes, reinterpret_cast<u8 *>(&scaledData2[0]), &availableSpace, allowRegeneration, &dataRegenerated, status);
         }
      ...
      // 3. Check for AO subsystem errors
      ...
      // 4. Update the run time for this generation
   ...
   if (! (aoErrored || dmaErrored))
   {
      printf("Finished generating continuous hardware-timed continuous analog updates for %.2f seconds.\n", runTime);
