...
class StringField extends AbstractField {
  ...
  copyin(buffer, value, initial_offset, cleanup_list) {
    const str = ref.allocCString(value, 'binary');
  ...
  copyout(buffer, initial_offset) {
    ...
    return ref.reinterpretUntilZeros(sbuf, 1).toString('binary');
  }
...
class ArrayField extends AbstractField {
  ...
  copyin(buffer, value, initial_offset, cleanup_list) {
    ...
    if (value.length !== this._num_elements){
      throw new Error('ArrayField copyin incorrect ' +
        'number of elements: ' + value.length +
        ' expected: ' + this._num_elements
      );
    }
...
class SequenceField extends AbstractField {
  ...
  copyin(buffer, value, initial_offset, cleanup_list) {
    ...
    if ((this._max_elements !== 0) && (len > this._max_elements)){
      throw new Error('SequenceField copyin incorrect ' +
        'number of elements: ' + len +
        ' expected: ' + this._max_elements
      );
    }
    if (this._max_elements > 0 && len > this._max_elements) {
    ...
    const contents = Buffer.alloc(len * sz);
    for (var i = 0; i < len; i++) {
...
class UnionField extends AbstractField {
  ...
  copyin(buffer, value, initial_offset, cleanup_list) {
    ...
    let case_name = this._union.getCaseName(value._d);
  ...
  copyout(buffer, initial_offset) {
    ...
    const case_name = this._union.getCaseName(u._d);
