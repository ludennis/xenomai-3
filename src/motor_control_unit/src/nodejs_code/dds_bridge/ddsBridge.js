...
async function sendControlMessage(message){
  ...
  try {
    ...
    publisherQos.partition = {names: 'NodejsRequestPartition'};
//    publisherQos.partition = {names: 'ControlCommandPartition'};
    ...
    subscriberQos.partition = {names: 'MotorResponsePartition'};
    const subscriber = participant.createSubscriber(subscriberQos);

    const readerQos = new dds.QoS({
      reliability: {kind: dds.ReliabilityKind.Reliable}});
    const reader = subscriber.createReader(motorTopic, readerQos);
