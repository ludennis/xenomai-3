...
function main(){
  readWriteIoTData().then(() => {
    console.log('=== IoTDataExample end');
    process.exit(0);
  }).catch((error) => {
...
async function readWriteIoTData() {

  console.log('=== IoTDataExample start');
  ...
  try {
    ...
    const typeSupport = typeSupports.get('DDS::IoT::IoTData');
    ...
    await integer64bitExample(participant, topic);
    await allUnionTypesExample(participant, topic);
...
async function allUnionTypesExample(participant, topic){
  ...
  const jsData = getAllUnionTypesSample(topic);
...
function getAllUnionTypesSample(topic){
  ...
  const typeSupport = topic.typeSupport;
  const IoTValue = typeSupport.getClass('DDS::IoT::IoTValue');
...
async function integer64bitExample(participant, topic){
